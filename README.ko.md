...
# `auto`와 `codex-auto-review`는 모델이 아니라 라우팅 결정을 가리키므로 가격이 존재할
# 수 없고, 해당 토큰은 $0.00으로 제출됩니다. 대부분의 턴을 `auto`로 보고하는 Kiro나
# Copilot을 쓴다면 다음 제출에서 기록된 지출이 낮아집니다. 이전 금액은 어떤 모델이
# 실행됐는지에 대한 추측에 가격을 매긴 값이었습니다.

# 가격이 필요한 사용량 대부분에 가격을 매길 수 없는 제출은 거부됩니다. 여기서는 가격
# 데이터 장애와 구분되지 않고, 서버는 해당 날짜에 이미 기록한 비용을 $0.00으로
# 덮어쓰기 때문입니다. 사용량에 정말로 공개 가격이 없다면 그대로 제출할 수 있습니다:
