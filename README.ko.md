...
8. **벤더 공식 요율** - 벤더가 직접 공개한 요율에서 가져온 하드코딩 가격 (Sakana, FriendliAI)
9. **퍼지 매칭** - 부분 모델 이름에 대한 단어 경계 매칭
...
**FriendliAI 가격**: `LGAI-EXAONE/K-EXAONE-236B-A23B`은 [FriendliAI가 직접 공개한 서버리스 요율표](https://friendli.ai/pricing)(입력 $0.20/1M, 출력 $0.80/1M, 캐시 입력 $0.10/1M)를 사용합니다. 어떤 업스트림 데이터셋도 이 모델을 싣고 있지 않기 때문입니다. OpenRouter와 models.dev에는 없고, LiteLLM에는 이 벤더의 `friendliai/meta-llama-3.1-*` 항목만 있습니다. `friendli/`와 `friendliai/` 표기 모두 해석되며, 세션이 제공자를 FriendliAI로 보고하는 경우에는 모델 ID만으로도 해석됩니다. 세션이 다른 제공자를 보고한다면 이 요율은 아예 적용하지 않습니다. 공개 가중치 모델이라 다른 호스트는 자기 요율로 과금하기 때문입니다. FriendliAI가 이 모델의 지원 종료일을 2026-08-20으로 공개했으므로 그 이후로는 계속 관리되기보다 오래된 값이 될 것으로 보시면 됩니다(그 이전에 기록된 세션은 계속 정확하게 계산됩니다). 참고로 llm-stats.com은 같은 모델을 입력 $0.60/1M, 출력 $1.00/1M으로 표기하지만, 실제로 이 엔드포인트에 과금하는 주체는 FriendliAI 본인이고 해당 집계 사이트는 수치의 출처를 밝히지 않았기 때문에 FriendliAI 자신의 수치를 사용합니다.

**BYOK 라우터**: `freerouter`는 과금 주체가 아니라 키를 직접 가져오는(BYOK) 게이트웨이입니다. 그래서 Tokscale은 모델 이름만 우연히 같은 무관한 제공자의 요율을 이런 경로에 적용하지 않습니다. `freerouter/nvidia/nemotron-3-ultra-550b-a55b`에 `kenari/nemotron-3-ultra-550b-a55b`의 $0이 적용되는 일은 없고, 제공자가 `freerouter`이고 모델 이름만 있는 경우도 마찬가지입니다. 대신 이런 경로도 그 모델 자신의 항목은 그대로 유지합니다. `freerouter/anthropic/claude-opus-4.6`은 Anthropic의 정가로 해석되고, 모델만 명시한 `freerouter/gpt-5.6` 같은 경로는 그 모델을 만든 곳의 항목, 즉 게이트웨이가 없을 때와 같은 가격으로 해석됩니다. 다른 벤더로 바뀌는 해석만 가격 없이 남습니다.
