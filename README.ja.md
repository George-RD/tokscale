...
# `auto` と `codex-auto-review` はモデルではなくルーティングの決定を指すため価格が
# 存在せず、そのトークンは $0.00 として送信されます。ほとんどのターンを `auto` と
# 報告する Kiro や Copilot を使っている場合、次回の送信で記録済みの支出が下がります。
# それまでの金額は、どのモデルが動いたかという推測に価格を付けたものでした。

# 価格が必要な使用量の大半に価格を付けられない送信は拒否されます。それは価格データの
# 障害と同じ見え方をし、サーバーはその日の記録済みコストを $0.00 で上書きするためです。
# 使用量に本当に公開価格が存在しない場合は、そのまま送信できます:
