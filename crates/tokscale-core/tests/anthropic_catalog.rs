...
/// unpriced LiteLLM rows filtered out, no cursor overrides needed for these
/// ids, and models.dev as the long-tail source. `None` when the cache is
/// absent.
///
/// This mirror has drifted from production before — it kept describing a
/// `github_copilot/` prefix filter for a release after that filter was gone.
/// When a pricing guard changes, change the mirror in the same commit or this
/// test quietly certifies the old behavior.
