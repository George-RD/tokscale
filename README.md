...
**FriendliAI Pricing**: `LGAI-EXAONE/K-EXAONE-236B-A23B` is priced from [FriendliAI's own serverless rate card](https://friendli.ai/pricing) ($0.20/1M input, $0.80/1M output, $0.10/1M cached input), because no upstream dataset carries it — it is absent from OpenRouter and models.dev, and LiteLLM has only `friendliai/meta-llama-3.1-*` rows. Both the `friendli/` and `friendliai/` spellings resolve, as does the model id on its own when the session reports FriendliAI as the provider. If the session reports a different provider, the rate is not applied at all — these are open weights, and another host bills for them at its own rate. FriendliAI publishes a deprecation date of 2026-08-20 for this model, so expect the entry to go stale after then rather than be maintained; sessions recorded before it still price correctly. Note that llm-stats.com lists the same model at $0.60/1M input and $1.00/1M output — FriendliAI's own figure is used because FriendliAI is the party that actually bills for the endpoint, while the aggregator cites no source for its numbers.

**BYOK Routers**: `freerouter` is a bring-your-own-key gateway rather than a billing entity, so Tokscale will not let a route through it take the rate of an unrelated provider that merely shares the model's name: `freerouter/nvidia/nemotron-3-ultra-550b-a55b` is not priced from `kenari/nemotron-3-ultra-550b-a55b` at $0, and neither is the bare model name under a `freerouter` provider. What such a route does keep is the model's own entry. `freerouter/anthropic/claude-opus-4.6` resolves to Anthropic's list rate, and a route that names only a model — `freerouter/gpt-5.6` — resolves to that model's author's entry, the same price it gets with no gateway in the picture. Only a resolution that swaps in some other vendor goes unpriced.
